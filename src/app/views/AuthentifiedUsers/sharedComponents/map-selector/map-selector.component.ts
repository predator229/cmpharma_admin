...
import 'leaflet-draw';
import {icon} from "leaflet"; // Import pour les types et extensions
...
export class MapSelectorComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  customIcon = new L.Icon({
    ...
    shadowUrl: 'assets/images/marker-icon-2x.png',
    iconSize: [10, 10],
    iconAnchor: [12, 41],
    popupAnchor: [1, -34],
    shadowSize: [41, 41]
  ...
  private createDefaultCityBounds(): CityBounds {
    const offset = 10 / 111; // 10 km en degrés
    return {
  ...
  private createDefaultZone(): void {
    ...
    const radius = 10 / 111; // 10 km de rayon en degrés
    const sides = 8;
